...
class BusisnessData{
  ...
  Future<bool> esCursoyUsuario (String uid, String curso) async {
    try{
      await _db.collection("usuarios").doc(uid).collection("cursos").doc(curso).get();
      return true;
    }
    catch (e){
      return false;
  ...
  Future<Curso?> getCurso(String nombre) async {
    ...
    try{
      nombrecurso = nombre;
      dia = DiaSemana.values.firstWhere((element) => element.toString() == curso['dia']);
      horainicio = TimeOfDay(hour: curso['horainicio'], minute: curso['minutoinicio']);
